...
import { getPetPassportByNumberPublic, type PetPassport } from '@/firebase/petPassportServicePublic';
...
const PetPassport = () => {
  ...
  useEffect(() => {
    const loadPassport = async () => {
      console.log('🔍 Loading passport for number:', passportNumber);
      
      if (!passportNumber) {
        console.error('❌ No passport number provided');
      ...
      try {
        ...
        console.log('📡 Fetching passport data from Firestore (no auth)...');
        const passportData = await getPetPassportByNumberPublic(passportNumber);
        
        console.log('📄 Passport data received:', passportData);
        
        if (!passportData) {
          console.error('❌ No passport found for number:', passportNumber);
          setError('Passaporto non trovato o non più attivo');
        } else {
          console.log('✅ Passport loaded successfully:', passportData.animalName);
        ...
        let errorMessage = 'Errore nel caricamento del passaporto';
        if ((err as any)?.code === 'permission-denied') {
          errorMessage = 'Accesso negato - problema di permessi Firebase';
        } else if ((err as any)?.code === 'unavailable') {
          errorMessage = 'Servizio temporaneamente non disponibile';
        }
        
        setError(`${errorMessage}: ${err instanceof Error ? err.message : 'Errore sconosciuto'}`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const downloadVCard = () => {
    ...
    // Separa nome e cognome (assume "Nome Cognome")
    const nameParts = passport.ownerName.trim().split(' ');
    const firstName = nameParts[0] || '';
    const lastName = nameParts.slice(1).join(' ') || '';

    // Genera contenuto vCard
    const vcardContent = [
      'BEGIN:VCARD',
      'VERSION:3.0',
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Connessione in corso...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-md mx-auto text-center">
          <div className="bg-white rounded-2xl shadow-lg p-8 border border-red-200">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              {error || 'Il passaporto richiesto non è stato trovato o non è più attivo.'}
            </p>
